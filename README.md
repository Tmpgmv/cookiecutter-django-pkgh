...
**`project_name_rus`** - Наименование проекта на русском языке. Может применяться в шаблонах (см. context_processors.py). Если надо что-то в кавычках: project_name_rus: "ООО &bsol;&quot;Аэропорт Таймыр&bsol;&quot;" . 
...
```bash
pg_dump --schema-only --dbname={{ cookiecutter.student_slug }}_{{ cookiecutter.project_slug }} --username={{ cookiecutter.db_username }} --password --host=localhost > schema.sql
```
